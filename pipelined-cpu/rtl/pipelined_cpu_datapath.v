...
module pipelined_cpu_datapath #(
  ...
  instruct_mem #(IMEM_WORDS) instruction_memory (
      .instAddress(IF_pc),  //word alignment will be performed in instruct_mem module
  ...
  data_mem #(DMEM_WORDS) data_memory (
      .dataAddress(EXMEM_aluResult),  //word alignment will be performed in data_mem module
      .writeData(EXMEM_storeData),
      .writeEnable(EXMEM_memWrite),
      .readEnable(EXMEM_memRead),
      .clk(clk),
      .readData(EXMEM_wordToLoad)
  );
